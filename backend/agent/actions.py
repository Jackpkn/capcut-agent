...
def execute_action(action: str, params: dict, project_path: str) -> str:
    ...
    if action == "add_transition":
        ...
        if result.get("skipped"):
            return f'Transition "{result["name"]}" already on clip — skipped'
        return f'Added transition "{result["name"]}" to clip'
    ...
    if action == "generate_image":
        raise RuntimeError(
            ...
            "Set IMAGE_GEN_API_KEY on the server (e.g. OpenAI/FAL) — then approve again."
        )
